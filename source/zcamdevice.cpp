...
int ZCAMDevice::ZGetFrame(void **pBuffer,size_t *nLen)
{
    ...
    {
        emit this->ZSigMsg("failed to DQBUF",Log_Msg_Error);
        return -1;
    }
    ...
    *nLen=this->m_IMGBuffer[getBuf.index].nLength;
    ...
    //write img to file.
    //    QString fileName=tr("%1-%2.yuv").arg(this->m_devName).arg(m_nCapTotal);
    //    QFile fileYUV(fileName);
    //    qDebug()<<fileName;
    //    if(fileYUV.open(QIODevice::WriteOnly))
    //    {
    //        fileYUV.write((const char*)this->m_IMGBuffer[getBuf.index].pStart,this->m_IMGBuffer[getBuf.index].nLength);
    //        fileYUV.flush();
    //        fileYUV.close();
    //    }
    //    m_nCapTotal++;
